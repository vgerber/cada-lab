...
export abstract class SketchInteractable<TargetType> {
  ...
  onMove: ((target: TargetType, movePosition: THREE.Vector3) => void) | null =
    null;
...
export class DragPoint<TargetType> extends SketchInteractable<TargetType> {
  ...
  constructor(
    target: TargetType,
    getPosition: (target: TargetType) => THREE.Vector3,
    onMove: (target: TargetType, movePosition: THREE.Vector3) => void,
    onMoveEnd?: (target: TargetType) => void,
  ) {
    super(target);
  ...
  clone(target?: TargetType): DragPoint<TargetType> {
    ...
    return new DragPoint<TargetType>(
      target,
      this.getPosition,
      this.onMove,
      this.onMoveEnd ? this.onMoveEnd : undefined,
    );
  }
