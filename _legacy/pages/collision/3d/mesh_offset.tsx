...
export default function LinePointDistance() {
  ...
  let normalA = sketchLineA.shape
    .dirNormalized()
    .cross(new THREE.Vector3(0, 0, 1))
    .multiplyScalar(-1);
  let normalB = sketchLineB.shape
    .dirNormalized()
    .cross(new THREE.Vector3(0, 0, 1));

  let normalLineA = new Line(
    "Normal A",
    lineA.a,
  ...
  let meanNormalVector = normalLineA
    .dirNormalized()
    .clone()
    .add(normalLineB.dirNormalized())
    .normalize();
  let offsetPosition = lineA.a
    .clone()
