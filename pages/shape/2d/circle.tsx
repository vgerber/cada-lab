...
export default function CirclePage() {
    let circle1 = new Circle("Circle 1", new THREE.Vector3(1, 1, 0), 1, 0, Math.PI * 2, 50);
    let circle2 = new Circle("Circle 2", new THREE.Vector3(1, 1, 0), 1, 0, Math.PI * 2, 50);

    function onPropertyChanged(property: Property) {
    }

    let sketchCircle2 = new SketchShape(circle2);
    sketchCircle2.addInteractable(new DragPoint(circle2, (target) => target.position, (target, position) => target.position.set(position.x, position.y, 0)));

    let sketchLine = new SketchShape(new Line("Test", new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 0, 0)));
