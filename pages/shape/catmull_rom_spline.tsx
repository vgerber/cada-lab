...
import { DashedLineProperties, Line, LineProperties } from "../../lib/drawing/shape/2d/line";
...
export default function ShapeCatmullRomSpline() {
    let [spline, setSpline] = useState(new CatmullRomSpline("CRS", [
    ...
    let lineP0Drag = new DragPoint(spline.points[0], (p) => p, (p, position) => (p.set(position.x, position.y, 0)));
    let lineP1Drag = new DragPoint(spline.points[1], (p) => p, (p, position) => (p.set(position.x, position.y, 0)));
    let lineP2Drag = new DragPoint(spline.points[2], (p) => p, (p, position) => (p.set(position.x, position.y, 0)));
    let lineP3Drag = new DragPoint(spline.points[3], (p) => p, (p, position) => (p.set(position.x, position.y, 0)));
    let lineP4Drag = new DragPoint(spline.points[4], (p) => p, (p, position) => (p.set(position.x, position.y, 0)));
    let lineP5Drag = new DragPoint(spline.points[5], (p) => p, (p, position) => (p.set(position.x, position.y, 0)));
    ...
    let line1 = new Line("P1", spline.points[spline.points.length - 2], spline.points[spline.points.length - 1]);
    ...
    return (
        <SketchBook sketch={new Sketch([sketchLine, new SketchShape(line0), new SketchShape(line1)])} onPropertyChanged={() => { }} onUpdate={(sketchShape) => onUpdateShape(sketchShape)} />
    )
}