...
export class Line implements Shape {
    ...
    clone(): Line {
        let line = new Line(this.name, this.a.clone(), this.b.clone());
    ...
    closestPoint(p: THREE.Vector3): THREE.Vector3 {
        let pa = p.clone().sub(this.a);

        let lineLength = this.length();
        let dir = this.dirNormalized();

        let lineMin = Math.min(0, lineLength);
        let lineMax = Math.max(0, lineLength);
        let t = Math.min(lineMax, Math.max(lineMin, pa.dot(dir)));
    ...
    getBoundingBox(): BoundingBox {
        return new BoundingBox(this.a.clone().min(this.b), this.a.clone().max(this.b));
    }
}